...
`https` wishes to trade with `http`, so `https` sends `http` a trade request:
...
`http` then sees the trade request, and accepts:

```
trade request received. begin trade with trade https
...
- You appear to only be able to offer one upgrade at a time.
