...
Players can obtain NPC locs from solving the puzzles in public scripts which end in ".((%Lpublic%))", which are obtained from the aforementioned quests or from looking them up in FULLSEC sectors using [[scripts.fullsec:((scripts.fullsec))]].
