...
> When called as a subscript in a script that is hosted by one player, it allows the upgrades of the script owner to be seen by a different user running the script.
...
The 'full' argument akes a boolean. When the value is set to `true` sys.upgrades_of_owner will display the full detail arguments of the owner's upgrades.
