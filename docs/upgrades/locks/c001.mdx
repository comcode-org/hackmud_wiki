...
c001 is a tier _N_ lock created by the CORE corporation.
...
`color_digit` is the length of the selected color. For example, the color `"lime"` has the `color_digit` value `4`.
